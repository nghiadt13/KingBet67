---
title: "Phase 5: Betting — Place Bet, Deposit Request, Bet History"
description: "Đặt cược (bottom sheet), yêu cầu nạp tiền, lịch sử cược. RPC place_bet + deposit-request flow."
status: partial
priority: P1
effort: 6h
...
1. **SQL:** RPC `place_bet` + deposit-request flow
2. **S-05:** Place Bet bottom sheet (từ Match Detail)
3. **S-09:** Deposit request section (thêm vào Profile)
...
| 3   | Deposit Request (S-09 Profile)            | Completed ✅ | 2-3h   | [phase-03](./phase-03-deposit.md)       |
...
| direct `deposit` from client no longer matches product requirement | Replace with request/approval flow. Only admin approval should mutate balance                                |
| Bottom sheet via `@gorhom/bottom-sheet`  | De-facto standard for RN bottom sheets. Already Expo-compatible                                              |
| `betStore.ts` separate from `matchStore` | Different concern — bet history, place bet state, deposit-request state                                      |
| Error mapping in frontend                | RPC RAISE EXCEPTION returns error codes → map to Vietnamese messages                                         |
| Refresh user balance after bet/deposit approval | Update `authStore.user.balance` after approved request is reflected                                          |
...
| **User can self-credit balance** if direct deposit UI remains | Critical | Remove direct `deposit` RPC usage from profile and replace with request creation |
